...
import Redis from "ioredis";
import { parser } from "./parser";
import { sendGmail } from "./gmail";  // Assume this is your Gmail sending function
...
async function processStage(zapRunId: string, currentStage: number) {
  try {
    ...
    if (currentAction?.AvailableAction.id === "gmail") {
      ...
      const from = parser(
        (currentAction.metadata as any)?.from as string,
        zapRunMetadata
      );
      const to = parser(
        (currentAction.metadata as any)?.to as string,
        zapRunMetadata
      );
      const subject = parser(
        (currentAction.metadata as any)?.subject as string,
        zapRunMetadata
      );
      const body = parser(
        (currentAction.metadata as any)?.body as string,
        zapRunMetadata
      );

      const result = await sendGmail(from, to, subject, body);
      if (result) {
...
async function main() {
  ...
  while (true) {
    const messages = await redis.xreadgroup(
      ...
      'BLOCK',
      0,  // Block indefinitely until a message is available
      'STREAMS',
      streamKey,
      '>'
    ) as [string, [string, string[]][]][];  // [stream, [id, fields][]] format
